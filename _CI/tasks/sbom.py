...
def uv_export(*extra_args: str) -> str:
    ...
    # S603 justification: `*extra_args` is caller-supplied flags, never shell text.
    # S607 justification: `uv` is resolved via PATH, deliberately.
    result = subprocess.run(  # noqa: S603
        ['uv', 'export', '--format', 'requirements-txt', '--no-hashes', '--no-header', *extra_args],  # noqa: S607
        capture_output=True,
        text=True,
        check=False,
    )
...
def origin_repo_url() -> str | None:
    ...
    try:
        result = subprocess.run(
            ['git', 'remote', 'get-url', 'origin'],  # noqa: S607 — resolved via PATH, deliberately
            capture_output=True,
            text=True,
            check=False,
        )
...
def uv_version() -> str:
    ...
    try:
        result = subprocess.run(
            ['uv', '--version'],  # noqa: S607 — resolved via PATH, deliberately
            capture_output=True,
            text=True,
            check=False,
        )
...
def lookup_pypi_license(name: str) -> LicenseRepository | None:
    """Return the declared license for an installed PyPI distribution.
...
# invalid-type-form justification, for every ty-ignore tag below: each return annotation
# here names a cyclonedx model class, and that class's own decorator resolves to
# `Union[Type[_T], Type[_JsonSerializable], Type[_XmlSerializable]]` under py-serializable's
# `from typing import Union as Intersection` stopgap — the annotation is unsound, not the
# value returned. See the override comment in pyproject.toml.jinja.
def lookup_pypi_hash(name: str, ver: str) -> HashType | None:  # ty: ignore[invalid-type-form]
...
def pypi_website_reference(name: str, ver: str) -> ExternalReference:  # ty: ignore[invalid-type-form]
...
def pypi_component(
    ...
    *,
    license_source: str,
) -> Component:  # ty: ignore[invalid-type-form]
...
def pipeline_external_reference(spec: PipelineComponent) -> ExternalReference | None:  # ty: ignore[invalid-type-form]
...
def pipeline_to_component(spec: PipelineComponent) -> Component:  # ty: ignore[invalid-type-form]
...
def project_authors(project_meta: dict) -> list[OrganizationalContact]:  # ty: ignore[invalid-type-form]
...
def project_supplier(project_meta: dict) -> OrganizationalEntity | None:  # ty: ignore[invalid-type-form]
...
def root_external_references() -> list[ExternalReference]:  # ty: ignore[invalid-type-form]
...
def build_root_component(project_meta: dict) -> Component:  # ty: ignore[invalid-type-form]
    """Build the project's own component, with bom-ref, vcs external_ref, and license."""
...
def build_build_environment_component() -> Component:  # ty: ignore[invalid-type-form]
...
def build_metadata_tools() -> ToolRepository:
...
def build_lifecycles() -> LifecycleRepository:
...
def build_bom() -> Bom:  # ty: ignore[invalid-type-form]
...
def validate_sbom(target: Path = SBOM_FILE) -> list[str]:
    ...
    """
    # Imported here rather than at module scope: `cyclonedx.validation.json` reaches
    # `jsonschema._format` and a lark grammar built at import time, 1.0s of the 1.2s this
    # module costs to import. Only this function validates.
    from cyclonedx.validation.json import JsonStrictValidator  # noqa: PLC0415
