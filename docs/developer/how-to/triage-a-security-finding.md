...
| `CVO260922_SECURITY_OVERRIDE` env var | **rejected** | no |
...
See [Environment variables and flags](../reference/environment-and-flags.md) for why this one
setting has three doors — a file, an environment variable and a flag — when nothing else does.
...
- **On pushes that change what you depend on** — the same workflow, triggered by `on.push.paths` for `uv.lock`, `pyproject.toml` and `.security-overrides`.
- **Before publishing** — `release.dist` audits before it builds. This is the gate that matters: publishing is when the outside world is exposed to these dependencies.
...
Deliberately *not* on every push, and not from `build`. An audit's answer depends on the
advisory database on the day it runs rather than on your branch, so a pull request that touches
a docstring would go red for a CVE published overnight that its author neither introduced nor
can fix — and a pipeline that fails for reasons outside the author's control is one people
learn to ignore. Bundling it into `build` had a sharper edge: a fresh advisory made it
impossible to build a wheel from code that built yesterday, blocking a hotfix on an unrelated
finding. Run it locally whenever you like with `./workflow.cmd secure.audit`, or alongside
everything else with `./workflow.cmd preflight --audit-dependencies`.
