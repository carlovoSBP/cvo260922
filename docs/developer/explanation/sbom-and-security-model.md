...
`./workflow.cmd secure.audit` runs [pip-audit](https://github.com/pypa/pip-audit) over every package version in your `uv.lock`, plus `_CI/lib/vendor.txt`, comparing each against the [PyPI advisory database](https://github.com/pypa/advisory-database). `--strict`, so a package the service cannot resolve fails the audit rather than being listed in a skip table under exit 0.
...
- **On pushes that change the dependency surface** — `uv.lock`, `_CI/lib/vendor.txt`, the
  ...
  commit carries `uv.lock`, and the commit that publishes is worth auditing anyway. Pushes
  only: a pull request from a fork triggers nothing here, so the audit that covers a fork's
  dependencies is the scheduled one, after the merge.
...
`./workflow.cmd secure.sbom-extract --write` generates a [CycloneDX](https://cyclonedx.org/) 1.7 bill of materials and writes it to `src/cvo260922/sbom.cdx.json`. Because that path is inside the package data tree, `uv build` automatically ships the SBOM **inside the wheel** — a downstream consumer unpacks the wheel and finds `cvo260922/sbom.cdx.json` alongside the Python modules.
...
- A downstream consumer can extract the SBOM from the wheel with `unzip -p <wheel> cvo260922/sbom.cdx.json` or `importlib.resources` — no separate artefact to track.
...
```bash
gh attestation verify cvo260922-<version>-py3-none-any.whl --repo <owner>/<repo>
```
